...
var _ = Describe("Types", func() {
	...
	Describe("custom Route type", func() {
		...
		It("marshals and unmarshals to JSON", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(jsonBytes).To(MatchJSON(`{ "dst": "1.2.3.0/24", "gw": "1.2.3.1", "mtu": 1500, "advmss": 1340, "priority": 100, "metadata": { "table": "0" } }`))
		...
		It("formats as a string with a hex mask", func() {
			Expect(example.String()).To(Equal(`{Dst:{IP:1.2.3.0 Mask:ffffff00} GW:1.2.3.1 MTU:1500 AdvMSS:1340 Priority:100 Metadata:map[table:0]}`))
		})
	})
